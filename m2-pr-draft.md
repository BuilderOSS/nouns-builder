...
**How:** Computed entirely client/API-side from existing subgraph data — no schema or codegen changes. A new `daoActivity` subgraph request, a pure `computeActiveMembers` helper, and a `useActiveMembers` SWR hook feed the UI. **Definition (configurable end-to-end, pending DAO validation):** a member is active if they voted on at least one of the DAO's last 5 completed (non-canceled) proposals OR placed an auction bid in the last 30 days. The list degrades gracefully — badges hide and the Active filter is disabled while activity loads or errors. The Active badge uses the shared `StatBadge` `positive` variant in its plain form — a light-green (`positiveDisabled`) tint with bold dark text, **no border and no dot** — matching the app's other status pills. (An earlier iteration added a hairline border and a leading green dot; that treatment was rejected in stakeholder review and reverted: the `statBadgeActive` style and the dot/variant code were removed and `StatBadgeProps.variant` narrowed back to `default | positive | negative | accent`.) v1 limitations: bidders are matched by address (a member bidding from a non-delegate wallet may be missed), vetoed proposals still count (only canceled are excluded), and there is no explicit Inactive state.
...
**What:** An `UrgencyAlerts` module pinned above the dashboard feed that surfaces what needs attention across a member's DAOs. Expanded post-feedback from three deadline countdowns into a **seven-kind taxonomy** spanning the auction and proposal lifecycles:

| Kind | Trigger (client-derived from payload) | Level | CTA |
|---|---|---|---|
| `AUCTION_NO_BIDS` | Auction ends within the critical window with no bids (`highestBid == null`) — suppresses plain `AUCTION_ENDING` | critical | Bid |
| `AUCTION_ENDING` | Current auction `endTime` within warning (→critical < 2h), has at least one bid | warning→critical | Bid |
| `VOTING_ENDING` | Active proposal, `voteEnd` within warning (→critical < 2h); shows a "You haven't voted" chip when applicable | warning→critical | Vote |
| `EXECUTION_EXPIRING` | Queued proposal, `expiresAt` within warning (→critical < 2h) — suppresses `PROPOSAL_EXECUTABLE` for that proposal | warning→critical | Execute |
| `VOTE_NEEDED` | Active proposal still open beyond the warning window, connected wallet hasn't voted — fires across the whole active window. No countdown; static "ends in N days" line | info | Vote |
| `PROPOSAL_EXECUTABLE` | Queued proposal, `executableFrom ≤ now`, no active `EXECUTION_EXPIRING` for it | info | Execute |
| `PROPOSAL_QUEUEABLE` | Proposal `state === Succeeded` (stalls forever until queued) | info | Queue |

**Why:** Members miss deadlines because the feed doesn't foreground time pressure — and high-leverage, currently-invisible states (a proposal that succeeded but nobody queued, or one that's queued and executable but nobody ran) stall the DAO indefinitely with no surface anywhere in the app. The three new `info` signals plus the `AUCTION_NO_BIDS` risk warning turn the strip from "which clock is running" into "what needs doing in my DAO right now."

**How:** A pure `deriveUrgencyAlerts` helper derives all seven kinds from dashboard DAO state with **configurable thresholds (pending DAO validation): warning < 24h, critical < 2h**; `info` kinds have no time threshold. All four new kinds are pure client-side derivations over fields already on every dashboard payload — **zero new queries, zero new RPC, zero subgraph change.** Two suppression rules keep the list clean (`AUCTION_NO_BIDS` replaces plain `AUCTION_ENDING`; `EXECUTION_EXPIRING` replaces `PROPOSAL_EXECUTABLE` for the same proposal). Critical/warning rows render a live countdown; `info` rows render a neutral `info` level variant (gray fill + low-contrast border, gray `text3` icon, no countdown). Rows stack in priority order — all critical, then warning, then info (newest first within info) — each with a deep link and a session-scoped dismiss. Wired into the apps/web `Dashboard` (single integration point covering home `/`, `/dashboard`, and the mobile feed tab). This track adds feed-ui test tooling (vitest/jsdom/testing-library, mirroring auction-ui) — the only lockfile change in this PR, an 18-line additive block scoped to feed-ui devDependencies.

**Future signals (discovery backlog, not implemented):** auction high bid below reserve, voting-opens-soon, voting-just-opened, new-proposal-created, quorum-at-risk, and auction-needs-settlement are client-cheap but held for DAO validation (threshold/heuristic or overlap-with-Feed judgment); proposal-outcome news (executed/defeated/vetoed/expired) and treasury/delegation events are backlog — they sit behind the server-side `ACTIVE_PROPOSAL_STATES` whitelist or have no reachable subgraph entity, so they need new data or a server change, not a client derivation.

**How to test:** With a connected wallet that has DAOs with auctions/proposals near their deadlines, open the dashboard and confirm urgency alerts appear above the feed in priority order, show a live countdown on critical/warning rows (none on info rows), switch styling under the 2h critical threshold, deep-link correctly, and stay dismissed for the session. Verify the suppression rules: a zero-bid auction in the critical window shows only `AUCTION_NO_BIDS`, and an expiring queued proposal shows only `EXECUTION_EXPIRING` (not also `PROPOSAL_EXECUTABLE`).
